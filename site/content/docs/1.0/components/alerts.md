...
Similarly, you can use [flexbox utilities]({{< docsref "/utilities/flex" >}}) and Icons to create alerts with icons. Depending on your icons and content, you may want to add more utilities or custom styles.
...
- Be sure you've loaded the alert plugin, or the compiled SuperStrap JavaScript.
...
As part of SuperStrap's evolving CSS variables approach, alerts now use local CSS variables on `.alert` for enhanced real-time customization. Values for the CSS variables are set via Sass, so Sass customization is still supported, too.
...
SuperStrap's alert plugin exposes a few events for hooking into alert functionality.
